...
Step5: Follow this to create commit command :
	 ...
	 For example : docker commit 6c31284597e6 myepuri/elasticsearch_vector:latest
...
For example: docker push myepuri/elasticsearch_vector:latest
...
Step8: Now we can build and push the image to docker hub, using the following command:
	...
	and then execute the below command to build a multi-platform image.
      ...
      For example: docker buildx build --platform linux/amd64,linux/arm64 -t myepuri/elasticsearch_vector:multi --push .
	  ...
	  myepuri/elasticsearch_vector:latest
